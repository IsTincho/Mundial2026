...
const EMPTY: LiveFeed = { live: {}, finals: {}, eventIds: {}, kickoffs: {}, progress: {} };

// `matchesRef` se lee en cada poll: así cubre también los partidos de
// eliminatorias, cuyos equipos cambian con la proyección de la tabla.
export function useLive(
  ...
  useEffect(() => {
    let alive = true;
    const tick = () => {
      fetchLive(matchesRef.current, groups).then((f) => {
        if (alive) setFeed(f);
      });
    };
    tick();
